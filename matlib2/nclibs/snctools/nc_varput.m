...
%         in case of an error.  Otherwise -1 is returned for failure,
%         0 for success.
%     
%   
% Examples:
...
% $Id: nc_varput.m,v 1.18 2006/01/25 14:22:29 johnevans007 Exp $
...
[var_name,var_type,nvdims,var_dim,natts, status]=mexnc('INQ_VAR',ncid,varid);
if status < 0 
	ncerr_msg = mexnc ( 'strerror', status );
...
%
% Find out who is the unlimited dimension.
[unlimdim, status] = mexnc ( 'inq_unlimdim', ncid );
if status < 0 
	mexnc ( 'close', ncid );
	ncerr_msg = mexnc ( 'strerror', status );
	msg = sprintf ( '%s:  ''inq_unlimdim'' operation failed on %s,  (%s).\n', mfilename, ncfile, ncerr_msg );
	handle_error ( msg, throw_exception );
	return;
end
...
if nvdims == 0
	...
	for n=1:nvdims,
		dimid=var_dim(n);
		[dim_name,dim_size,status]=mexnc('INQ_DIM',ncid,dimid);
		if status < 0 
			ncerr_msg = mexnc ( 'strerror', status );
...
nc_start = zeros(size(nc_count));
nc_stride = ones(size(nc_count));
...
have_scale_factor = 0;
have_add_offset = 0;
[sf_datatype, att_len, status] = mexnc('INQ_ATT', ncid, varid, 'scale_factor' );
if ( status == 0 )
	have_scale_factor = 1;
end
[ao_datatype, att_len, status] = mexnc('INQ_ATT', ncid, varid, 'add_offset' );
if ( status == 0 )
	have_add_offset = 1;
end
...
[att_datatype, att_len, status] = mexnc('INQ_ATT', ncid, varid, '_FillValue' );
if ( status == 0 )
