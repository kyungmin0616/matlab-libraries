...
nc('time') = 0; % opt.time; %% (record dimension)
...
if opt.fe == 1
nc{'irond'} = ncfloat('time', 's_rho', 'eta_rho', 'xi_rho'); %% 1726200 elements.
nc{'irond'}.long_name = ncchar('phytoplankton concentration');
nc{'irond'}.units = ncchar('millimole_nitrogen meter-3');
nc{'irond'}.time = ncchar('ocean_time');
nc{'irond'}.coordinates = ncchar('x_rho y_rho');
nc{'irond'}.field = ncchar('phytoplankton, scalar, series');
 
nc{'ironp'} = ncfloat('time', 's_rho', 'eta_rho', 'xi_rho'); %% 1726200 elements.
nc{'ironp'}.long_name = ncchar('phytoplankton concentration');
nc{'ironp'}.units = ncchar('millimole_nitrogen meter-3');
nc{'ironp'}.time = ncchar('ocean_time');
nc{'ironp'}.coordinates = ncchar('x_rho y_rho');
nc{'ironp'}.field = ncchar('phytoplankton, scalar, series');
end
...
if isfield(opt, 'timeval')
  nc{'ocean_time'}(:) = opt.timeval;
  nc{'scrum_time'}(:) = opt.timeval;
end
