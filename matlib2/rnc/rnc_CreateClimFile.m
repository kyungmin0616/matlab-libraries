...
% Default options
   ...
   opt.fe=0;
   opt.tclm_time=12;
   opt.tclm_timeVal=[15:30:360];
   opt.tclm_time_cycle=360;
   opt.sclm_time=12;
   opt.sclm_timeVal=[15:30:360];
   opt.sclm_time_cycle=360;
   opt.ssh_time=12;
   opt.ssh_timeVal=[15:30:360];
   opt.ssh_time_cycle=360;
   opt.uclm_time=12;
   opt.uclm_timeVal=[15:30:360];
   opt.uclm_time_cycle=360;
   opt.vclm_time=12;
   opt.vclm_timeVal=[15:30:360];
   opt.vclm_time_cycle=360;
   opt.ocean_time=12;
   opt.ocean_timeVal=[15:30:360];
   opt.ocean_time_cycle=360;
   opt.ptracer=0;
   opt.nudge=0;

   opt.climvars={'tclm_time' 'sclm_time' 'ssh_time' 'uclm_time' 'vclm_time' 'ocean_time'};
...
if opt.ocean_time_cycle ~= 0
  nc{'ocean_time'}.cycle_length = ncdouble(opt.ocean_time_cycle);
end
...
if opt.fe == 1
nc{'irond'} = ncfloat('ocean_time', 's_rho', 'eta_rho', 'xi_rho'); %% 1726200 elements.
nc{'irond'}.long_name = ncchar('available disolved iron concentration');
nc{'irond'}.units = ncchar('millimole_nitrogen meter-3');
nc{'irond'}.time = ncchar('ocean_time');
nc{'irond'}.coordinates = ncchar('x_rho y_rho');
nc{'irond'}.field = ncchar('iron, scalar, series');
 
nc{'ironp'} = ncfloat('ocean_time', 's_rho', 'eta_rho', 'xi_rho'); %% 1726200 elements.
nc{'ironp'}.long_name = ncchar('phytop. associated iron concentration');
nc{'ironp'}.units = ncchar('millimole_nitrogen meter-3');
nc{'ironp'}.time = ncchar('ocean_time');
nc{'ironp'}.coordinates = ncchar('x_rho y_rho');
nc{'ironp'}.field = ncchar('iron, scalar, series');
end

end
%          Nudging section
if opt.nudge == 1
nc{'temp_nudge'} = ncdouble('tclm_time', 's_rho', 'eta_rho', 'xi_rho'); %% 2304000 elements.
nc{'temp_nudge'}.long_name = ncchar('potential temperature');
nc{'temp_nudge'}.units = ncchar('Celsius');
nc{'temp_nudge'}.field = ncchar('temperature, scalar, series');
nc{'temp_nudge'}.time = ncchar('tclm_time');
    
nc{'salt_nudge'} = ncdouble('sclm_time', 's_rho', 'eta_rho', 'xi_rho'); %% 2304000 elements.
nc{'salt_nudge'}.long_name = ncchar('salinity');
nc{'salt_nudge'}.units = ncchar('PSU');
nc{'salt_nudge'}.field = ncchar('salinity, scalar, series');
nc{'salt_nudge'}.time = ncchar('sclm_time');

if opt.npzd == 1
nc{'NO3_nudge'} = ncfloat('ocean_time', 's_rho', 'eta_rho', 'xi_rho'); %% 951090 elements.
nc{'NO3_nudge'}.long_name = ncchar('nitrate concentration');
nc{'NO3_nudge'}.units = ncchar('millimole_N03 meter-3');
nc{'NO3_nudge'}.time = ncchar('ocean_time');
nc{'NO3_nudge'}.coordinates = ncchar('x_rho y_rho');
nc{'NO3_nudge'}.field = ncchar('NO3, scalar, series');

nc{'phytoplankton_nudge'} = ncfloat('ocean_time', 's_rho', 'eta_rho', 'xi_rho'); %% 951090 elements.
nc{'phytoplankton_nudge'}.long_name = ncchar('phytoplankton concentration');
nc{'phytoplankton_nudge'}.units = ncchar('millimole_nitrogen meter-3');
nc{'phytoplankton_nudge'}.time = ncchar('ocean_time');
nc{'phytoplankton_nudge'}.coordinates = ncchar('x_rho y_rho');
nc{'phytoplankton_nudge'}.field = ncchar('phytoplankton, scalar, series');

nc{'zooplankton_nudge'} = ncfloat('ocean_time', 's_rho', 'eta_rho', 'xi_rho'); %% 951090 elements.
nc{'zooplankton_nudge'}.long_name = ncchar('zooplankton concentration');
nc{'zooplankton_nudge'}.units = ncchar('millimole_nitrogen meter-3');
nc{'zooplankton_nudge'}.time = ncchar('ocean_time');
nc{'zooplankton_nudge'}.coordinates = ncchar('x_rho y_rho');
nc{'zooplankton_nudge'}.field = ncchar('zooplankton, scalar, series');

nc{'detritus_nudge'} = ncfloat('ocean_time', 's_rho', 'eta_rho', 'xi_rho'); %% 951090 elements.
nc{'detritus_nudge'}.long_name = ncchar('detritus concentration');
nc{'detritus_nudge'}.units = ncchar('millimole_nitrogen meter-3');
nc{'detritus_nudge'}.time = ncchar('ocean_time');
nc{'detritus_nudge'}.coordinates = ncchar('x_rho y_rho');
nc{'detritus_nudge'}.field = ncchar('detritus, scalar, series');

if opt.fe == 1
nc{'irond_nudge'} = ncfloat('ocean_time', 's_rho', 'eta_rho', 'xi_rho'); %% 1726200 elements.
nc{'irond_nudge'}.long_name = ncchar('available disolved iron concentration');
nc{'irond_nudge'}.units = ncchar('millimole_nitrogen meter-3');
nc{'irond_nudge'}.time = ncchar('ocean_time');
nc{'irond_nudge'}.coordinates = ncchar('x_rho y_rho');
nc{'irond_nudge'}.field = ncchar('iron, scalar, series');

nc{'ironp_nudge'} = ncfloat('ocean_time', 's_rho', 'eta_rho', 'xi_rho'); %% 1726200 elements.
nc{'ironp_nudge'}.long_name = ncchar('phytop. associated iron concentration');
nc{'ironp_nudge'}.units = ncchar('millimole_nitrogen meter-3');
nc{'ironp_nudge'}.time = ncchar('ocean_time');
nc{'ironp_nudge'}.coordinates = ncchar('x_rho y_rho');
nc{'ironp_nudge'}.field = ncchar('iron, scalar, series');
end
end
end    %  end of nudging
...
for i=1:length(opt.climvars)
  eval(['tmp=opt.', opt.climvars{i} ,'Val;']);
  nc{ opt.climvars{i} }(:) = tmp;
end
