function [c99, c95, c12, signi, cpdf_xaxis, cpdf_values]=ctana_red_signi(s1,s2,NUM,varargin);
...
% Clear NaN values
in=find(~isnan(s1)); s1=s1(in); 
in=find(~isnan(s2)); s2=s2(in); 
...
% Plot timeseries
subplot(2,1,1)
plot(s1,'b'); hold on; plot(cfac*s2,'r'); datetick;
s11=s1; s22=s2;
