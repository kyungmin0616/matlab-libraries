...
%   $Revision: 5.33 $  $Date: 1998/07/29 18:16:23 $
...
if nargin>=3 & strcmp(varargin{1},'manual'),
  manual = 1;
  inargs(1)=1;
end
...
if ~manual, 
% MANU
  labels2 = getlabels(CS);
  labels2;
  iii=length(labels2(1,:));
  if iii > 5 , iii=5;end
  for i=1:length(labels2(:,1))
  labels(i,:) = labels2(i,1:iii);
  end
  labels  ;
