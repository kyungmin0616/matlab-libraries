...
crgb=[0.15000   0.00000   0.37000
  ...
  if grtype == 'p'
    ...
    pcolorjw(lonp.*cmask,latp.*cmask,field.*maskp.*cmask)
  ...
  if grtype == 'v'
    ...
    pcolorjw(lonv.*cmask,latv.*cmask,field.*maskv.*cmask)
    colormap(rgb); colorbar; shading interp;
    hold on
if indc == 1,     [cs,h]=contour(lonv.*cmask,latv.*cmask,field.*maskv.*cmask,numCon,'k');end
  ...
  if grtype == 'u'
    ...
    pcolorjw(lonu.*cmask,latu.*cmask,field.*masku.*cmask)
    colormap(rgb); colorbar; shading interp;
    hold on
if indc == 1,     [cs,h]=contour(lonu.*cmask,latu.*cmask,field.*masku.*cmask,numCon,'k');end
  ...
  if grtype == 'r'
    pcolorjw(lonr.*cmask',latr.*cmask',field.*maskr.*cmask');
    shading interp;
