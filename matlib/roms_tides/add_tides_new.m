...
%if nargin<1,
  ...
  Gname ='usw15_grid.nc';
  Ntides=10;                         % Nb of tidal constituents in output
  tidefile='TPXO.5_tides.mat';       % if iSAVE defined
%end;
...
iSAVE    =1;
iPLOT    =0;
iNODAL   =1;      % Correct phase and amplitude using astron. 
...
% TPXO files
%
Hname ='/disk27/patrickm/TIDES/TPXO.5/H_tpxo/h_TPXO.5.1.ot';
UVname='/disk27/patrickm/TIDES/TPXO.5/UV_tpxo/u_TPXO.5.1';
Bname ='/disk27/patrickm/TIDES/TPXO.5/UV_tpxo/grid';
...
thetalimUV = fread(tpid,[1 2],'float32');
philimUV   = fread(tpid,[1 2],'float32');
...
ioff=183.5;
joff=-1;
llmima2=find((hlon+ioff<=lonmax)&(hlon+ioff>=lonmin) & ...
             (hlat+joff<=latmax)&(hlat+joff>=latmin));
hz_sub=reshape(hz(llmima2),n_sub,m_sub);
%hz_sub=nozero(hlon_sub,hlat_sub,hz_sub);
hz_sub(hz_sub<50)=50;
...
%  if TPXO_bathy, warning off; uv=uv./hz; warning on; end;%  Patrick
	
  uv_sub=reshape(uv(llmima),n_sub,m_sub);
  uv_sub=nozero(hlon_sub,hlat_sub,uv_sub);
  warning off; uv_sub=uv_sub./hz_sub; warning on;
...
% Convert transport to Velocity - Patrick
  if ~TPXO_bathy,
    ui(:,:,itide)=ui(:,:,itide)./h;
    vi(:,:,itide)=vi(:,:,itide)./h;
  end;
...
if (iSAVE), 
  eval(['save ',tidefile,' Tide ui_amp ui_pha vi_amp vi_pha']);
end;
