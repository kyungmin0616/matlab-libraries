function rgrd_ExtractForcing(forcfile,grd,forcfile2,grd1)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
rgrd_create_forcing(child_frc,parent_frc,child_grd,title,smst,...
                         shft,swft,srft,sstt,ssst,smsc,...
                         shfc,swfc,srfc,sstc,sssc)
% rgrd_create_forcing.m
%
% parent indices
%
...
for tindex=1:length(smst)
  tindex
  rgrd_nestvar3d(np,nc,igrd_u,jgrd_u,ichildgrd_u,jchildgrd_u,'sustr',tindex)
  rgrd_nestvar3d(np,nc,igrd_v,jgrd_v,ichildgrd_v,jchildgrd_v,'svstr',tindex)
end
disp('Heat flux ...')
for tindex=1:length(shft)
  rgrd_nestvar3d(np,nc,igrd_r,jgrd_r,ichildgrd_r,jchildgrd_r,'shflux',tindex)
end
disp('Freshwater flux...')
for tindex=1:length(swft)
  rgrd_nestvar3d(np,nc,igrd_r,jgrd_r,ichildgrd_r,jchildgrd_r,'swflux',tindex)
end
disp('SST and heat flux correction ...')
for tindex=1:length(sstt)
  rgrd_nestvar3d(np,nc,igrd_r,jgrd_r,ichildgrd_r,jchildgrd_r,'SST',tindex)
  rgrd_nestvar3d(np,nc,igrd_r,jgrd_r,ichildgrd_r,jchildgrd_r,'dQdSST',tindex)
end
disp('SSS ...')
for tindex=1:length(ssst)
  rgrd_nestvar3d(np,nc,igrd_r,jgrd_r,ichildgrd_r,jchildgrd_r,'SSS',tindex)
end
disp('Shortwave radiation ...')
for tindex=1:length(srft)
  rgrd_nestvar3d(np,nc,igrd_r,jgrd_r,ichildgrd_r,jchildgrd_r,'swrad',tindex)
end
result=close(np);
result=close(nc);
disp(' ')
