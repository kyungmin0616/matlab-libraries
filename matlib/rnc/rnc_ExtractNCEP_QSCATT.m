function  qscb = rnc_NCEP_QSCATT_Forcing(lonr,latr, mydatenum, avg_freq, varargin);
% get ncep winds.
...
for TIMEINDEX=1: length(ctlf.datenum)
   ...
   if         strcmp(freq,'daily')
	 ...
	 if length(in)==0
        ...
        qscb.svstr(:,:,TIMEINDEX)=rnt_loadvar_segp(ctl,in,'svstr',i,j,1);
        qscb.sustr(:,:,TIMEINDEX)=rnt_loadvar_segp(ctl,in,'sustr',i,j,1);       
	  qscb.avg_type='daily';

   elseif     strcmp(freq,'monthly')
	  in=find(ctl.month == ctlf.month(TIMEINDEX) & ctl.year == ctlf.year(TIMEINDEX));	  
        qscb.svstr(:,:,TIMEINDEX)=mean(rnt_loadvar_segp(ctl,in,'svstr',i,j,1),3);
        qscb.sustr(:,:,TIMEINDEX)=mean(rnt_loadvar_segp(ctl,in,'sustr',i,j,1),3);        
