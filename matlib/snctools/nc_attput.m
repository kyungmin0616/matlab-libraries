...
% $Id: nc_attput.m,v 1.13 2005/08/01 12:58:51 johnevans007 Exp $
...
if ( (ischar(varname)) & ( strcmp(lower(varname),'global') | strcmp(lower(varname),'nc_global') ) )
	[varid, status] = mexnc ( 'inq_varid', ncid, varname );
	if ( status == 0 )
		%
		% There ***WAS*** a variable called 'GLOBAL'.  
...
switch class(attval)
	...
	case 'double'
		status = mexnc ( 'put_att_double', ncid, varid, attribute_name, 'double', length(attval), attval);
	case 'single'
		status = mexnc ( 'put_att_float', ncid, varid, attribute_name, 'float', length(attval), attval);
	case 'int32'
		status = mexnc ( 'put_att_int', ncid, varid, attribute_name, 'int', length(attval), attval);
	case 'int16'
		status = mexnc ( 'put_att_short', ncid, varid, attribute_name, 'short', length(attval), attval);
	case 'int8'
		status = mexnc ( 'put_att_schar', ncid, varid, attribute_name, 'byte', length(attval), attval);
	case 'uint8'
		status = mexnc ( 'put_att_uchar', ncid, varid, attribute_name, 'byte', length(attval), attval);
	case 'char'
		status = mexnc ( 'put_att_text', ncid, varid, attribute_name, 'char', length(attval), attval);
	otherwise
		mexnc ( 'close', ncid );
