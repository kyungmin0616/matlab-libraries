function [sigh2, freq , period, sigh2_AR1, sigh2_AR1_90, phase ] = myspec(sig,dt,nchunk);
...
for i=0:nchunk-1
   chunk=[1:dN]+dN*i;	
   sig1=sig(chunk);
   [sigh2_tmp, freq , period, sigh2_AR1_tmp, sigh2_AR1_90_tmp, fft_sig_tmp, phase_tmp ] = local_spec(sig1,dt,dof);	
   if i==0  
	  sigh2=sigh2_tmp;
...
function [sigh2, freq , period, sigh2_AR1, sigh2_AR1_90, fft_sig, phase ] = local_spec(sig,dt,dof);
N=length(sig);
sig=reshape(sig,[N 1]);
...
function [sigh2, freq , period, sigh2_AR1, sigh2_AR1_90 ] = local_spec2(sig,dt,dof);
N=length(sig);
sig=reshape(sig,[N 1]);
